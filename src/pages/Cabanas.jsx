...
export function Cabanas(props) {
  const cabanas = props.cabanas, reservas = props.reservas;

  function countRes(id) { return reservas.filter((r) => r.cabanaId === id).length; }

  function addCab() {
    const usados = cabanas.map((c) => c.color);
    const color = FDL.COLOR_ORDER.find((c) => usados.indexOf(c) === -1) || "arena";
    props.onAdd({ id: "c" + Date.now().toString(36), nombre: "Cabaña " + (cabanas.length + 1), maxPersonas: 4, color });
  }
              ...
              <div className="cab-card-top">
                ...
                {cabanas.length > 1 && (
                  <button className="icon-btn subtle" title="Eliminar cabaña"
                    onClick={() => { if (confirm("¿Eliminar " + c.nombre + "? Esto no borra sus reservas.")) props.onDelete(c.id); }}>
                    <Icon.trash size={16} />
                  </button>
                )}
              </div>
              <label className="cab-lbl">Nombre</label>
              <input className="inp" value={c.nombre} onChange={(e) => props.onUpdate(c.id, "nombre", e.target.value)} />

              <div className="row-2" style={{ marginTop: 12 }}>
                <div>
                  <label className="cab-lbl">Máx. personas</label>
                  <div className="stepper">
                    <button type="button" onClick={() => props.onUpdate(c.id, "maxPersonas", Math.max(1, (c.maxPersonas || 1) - 1))}>–</button>
                    <span>{c.maxPersonas}</span>
                    <button type="button" onClick={() => props.onUpdate(c.id, "maxPersonas", (c.maxPersonas || 0) + 1)}>+</button>
                  </div>
                </div>
                <div>
                  <label className="cab-lbl">Color</label>
                  <div className="color-row">
                    {FDL.COLOR_ORDER.map((ck) => {
                      const cc = FDL.CABANA_COLORS[ck];
                      const active = c.color === ck;
                      return (
                        <button key={ck} type="button" title={cc.label}
                          onClick={() => props.onUpdate(c.id, "color", ck)}
