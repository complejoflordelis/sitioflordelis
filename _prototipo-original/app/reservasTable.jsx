      ...
      <div className="page-head">
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          <div className="search-box">
            <Icon.search size={16} />
            <input placeholder="Buscar familia, ciudad…" value={q} onChange={function(e){ setQ(e.target.value); }} />
          </div>
        ...
        <table className="tx-table">
          ...
          <tbody>
            {filtered.map(function (r) {
              ...
              return (
                <tr key={r.id}>
                  <td><input type="date" className="cell" value={r.fechaVenta} onChange={function(e){ upd(r.id,"fechaVenta",e.target.value); }} /></td>
                  <td className="cell-cab">
                    <select className="cell sel" value={r.cabanaId} onChange={function(e){ upd(r.id,"cabanaId",e.target.value); }} style={{ color: col.ink, background: col.soft }}>
                      {cabanas.map(function(cc){ return <option key={cc.id} value={cc.id}>{cc.nombre}</option>; })}
                    </select>
                  </td>
                  <td><input type="date" className="cell" value={r.inicioEstadia} onChange={function(e){ upd(r.id,"inicioEstadia",e.target.value); }} /></td>
                  <td><input type="date" className="cell" value={r.finEstadia} onChange={function(e){ upd(r.id,"finEstadia",e.target.value); }} /></td>
                  <td className="calc num">{FDL.noches(r)}</td>
                  <td className="calc">{m ? FDL.MESES_ABR[m.getMonth()] : "—"}</td>
                  <td><input className="cell" value={r.nombre} onChange={function(e){ upd(r.id,"nombre",e.target.value); }} /></td>
                  <td className="calc num">{FDL.pax(r)}</td>
                  <td><input className="cell num" inputMode="numeric" value={r.menores} onChange={function(e){ upd(r.id,"menores",e.target.value.replace(/[^\d]/g,"")); }} /></td>
                  <td><input className="cell wide-cell" value={r.ciudadOrigen} onChange={function(e){ upd(r.id,"ciudadOrigen",e.target.value); }} /></td>
                  <td><input className="cell" value={r.celular} onChange={function(e){ upd(r.id,"celular",e.target.value); }} /></td>
                  <td className="num">
                    <div className="money-cell">
                      <input className="cell num" inputMode="numeric" value={r.importeIngresado} onChange={function(e){ upd(r.id,"importeIngresado",e.target.value.replace(/[^\d]/g,"")); }} />
                  ...
                  <td className="num"><input className="cell num" inputMode="numeric" value={r.anticipo} onChange={function(e){ upd(r.id,"anticipo",e.target.value.replace(/[^\d]/g,"")); }} /></td>
                  <td><input className="cell" placeholder="—" value={r.pagadoDepositoA} onChange={function(e){ upd(r.id,"pagadoDepositoA",e.target.value); }} /></td>
                  <td><input type="date" className="cell" value={r.fechaDeposito} onChange={function(e){ upd(r.id,"fechaDeposito",e.target.value); }} /></td>
                  <td className={"calc num" + (FDL.saldo(r)>0?" pend":"")}>{FDL.fmtMoney(FDL.saldo(r))}</td>
                  <td><input className="cell" placeholder="—" value={r.pagadoSaldoA} onChange={function(e){ upd(r.id,"pagadoSaldoA",e.target.value); }} /></td>
                  <td><input type="date" className="cell" value={r.fechaPagoCliente} onChange={function(e){ upd(r.id,"fechaPagoCliente",e.target.value); }} /></td>
                  <td className="num"><input className="cell num" inputMode="numeric" value={r.comision} onChange={function(e){ upd(r.id,"comision",e.target.value.replace(/[^\d]/g,"")); }} /></td>
                  <td className="calc num strong">{FDL.fmtMoney(FDL.saldoFlorDeLis(r))}</td>
                  <td className="sticky-r">
                    <button className="row-del" title="Eliminar reserva" onClick={function(){ if (confirm("¿Eliminar la reserva de " + r.nombre + "?")) props.onDelete(r.id); }}><Icon.trash size={15} /></button>
                  </td>
                </tr>
              );
            })}
